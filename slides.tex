...
\begin{frame}
	...
	\begin{flushleft}
		...
		Bitbucket, and anything else in the cloud\ldots and a lot of other stuff not
		in the cloud). They are not an introduction to Go. Most of them were cribbed
		from Rob Pike, Andrew Gerrand, and others. The Gopher logo is by Renee
		French.
	\end{flushleft}
...
\begin{frame}
	\frametitle{The Zen of \xcancel{Python} Go}
	\begin{tikzpicture}[remember picture,overlay]
	...
	\begin{itemize}
		...
		\item Although that way may not be obvious at first unless you're \cancel{Dutch} Rob Pike.
...
\begin{frame}
	...
	\begin{flushleft}
		\only<3->{As of Go 1: the language is done.}
		\only<4->{Finished.}
		\only<5>{Complete.}
	\end{flushleft}
\end{frame}
...
\begin{frame}
	...
	\end{flushleft}
	\begin{itemize}
		\item Tests reside in the same package as the thing they're testing.
		\item Tests are not linked in when building by default.
		\item The test tool is just a shortcut to build a special test app with your code/tests linked in.
		\item Test binaries easy to debug with GDB or your favorite debugger.
	\end{itemize}
\end{frame}
